...
export function getWbsColor(wbs) { for (const [k,c] of Object.entries(WBS_COLORS)) { if (wbs.toLowerCase().includes(k.toLowerCase())) return c; } return '#6B7280'; }
export function generateSampleData() {
  ...
  [1,2,3,4].forEach((bldg, bIdx) => {
    ...
    phases.forEach(ph => { ph.tasks.forEach(t => {
      activities.push({id:t.code,code:t.code,name:t.name,wbs:ph.wbs,wbsName:ph.name,building:bldg,duration:t.dur,startDay:t.start,
